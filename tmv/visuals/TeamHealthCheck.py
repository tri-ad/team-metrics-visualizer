...
def thc_session_picker(
    ...
    for control_i in [0, 1]:
        ...
        selected_value = slicers_state.load_slicer_value(
...
def callback_thc_session_picker_state_saving(app, picker_id, picker_compare_id):
    slicers_state.callback_slicer_state_saving(
        app, "thc_session_picker_0", picker_id, "value"
    )
    slicers_state.callback_slicer_state_saving(
...
def callback_update_thc_visuals_by_teams_and_sessions(
    ...
    thc_table: Optional[THCResultTableController] = None,
    thc_graph_id: str = "",
    thc_graph: Optional[THCTrendGraphController] = None,
):
